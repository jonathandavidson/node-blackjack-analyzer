...
export default function basicStrategy (hand, dealerCard, doubleDownIsAllowed, surrenderIsAllowed) {
  ...
  if (isPair(hand) && value !== 20 && value != 10) {
    ...
    if (!hand.isSoft() && dealerValue >= 2 && dealerValue <= 6) {
      action = stand;
    } else if (hand.isSoft() && [5, 6].includes(dealerValue)) {
      action = doubleDownIsAllowed ? doubleDown : hit;
    }
  } else if (value === 15) {
    if (!hand.isSoft()) {
